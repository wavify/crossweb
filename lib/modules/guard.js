    ...
    if (credential.type == 'plain') {
      ...
      if (found) {
        if (found.password == password) {
          ...
          callback(new Error(FrameworkError.AUTHENTICATE_WRONG_PASSWORD));
        }
      } else {
        callback(new Error(FrameworkError.AUTHENTICATE_NO_USER));
      }
    
    } else {
      callback(new Error(FrameworkError.AUTHENTICATE_INVALID_TYPE));
    }
  }
