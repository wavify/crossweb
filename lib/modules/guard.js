...
var Guard = function Guard (configPath) {
  ...
  if (guardConfig.authenticator) {
    ...
    if (fs.existsSync(authenticatorFile)) {
...
/**
 * Make session expire and remove from store.
 *
 * @param {String} id, session id
 * @param {Function} callback
 */
Guard.prototype.expire = function (id, callback) {
  ...
  sessions.get(id, function (error, storedSession) {
    ...
    if (error) {
      ...
      sessions.expire(storedSession, callback);
    }
 ...
 *
 */
_Sessions.prototype.expire = function (id, callback) {
  /* Their is nouse to remove session here because it's store
   * in client side.
   *
   */
  callback();
