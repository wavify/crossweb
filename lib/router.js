...
Router.parse = function (configPath) {
  ...
  for (var index = 0; index < filters.length; index++) {
    ...
    var filterPath = path.join(path.dirname(configPath), 'filters', filters[index] + '.js');
    if (!path.existsSync(filterPath)) {
      filterPath = path.join(__dirname, 'filters', filters[index] + '.js');
      if (!path.existsSync(filterPath)) {
        logger.error ('Filter ' + filters[index] + ' does not exists');
        continue;
      }
    }
    
    try {
      var filter = require(filterPath)[filters[index]];
      ...
      if (filter.check && filter.fail) {
        ...
        logger.error (filters[index] + ' is not filter object');
      }
      
    } catch (e) {
      logger.debug (e);
      logger.error ('Cannot load filter ' + filters[index]);
    }
